export default (missingMember) => {
  return `<div class="element-item ${missingMember.party} ${missingMember.state} ${missingMember.displayType} ${missingMember.ahca ? 'ahca' :''} ${missingMember.taxBill ? 'taxBill': ''} ${missingMember.missingMember? 'missingMember': ''}">
  <div class="card card-representative ${missingMember.party}">
    <div class="card-header">
      <div class="row">
      <div class="col-xs-4 col-sm-3 no-padding p-0">
        <img src="https://www.govtrack.us/data/photos/${missingMember.govtrack_id}-100px.jpeg" onerror="$(this).remove()" class="grayscale" />
      </div>
