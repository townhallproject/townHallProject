...
import { firestore } from "../../../scripts/lib/firebasedb";
...
export default class ZipSearch extends React.Component {
  static handleZipToDistrict(zipToDistrictArray) {
    ...
    if (!eventHandler.checkStateName(federal.state)) {
      return zipLookUpHandler.zipErrorResponse(
        "That zipcode is not in " +
          stateView.state +
          '. Go back to <a href="/">Town Hall Project U.S.</a> to search for events.'
      );
    }
  ...
  static getLookupArray() {
    if (stateView.state) {
      return [
        "/zipToDistrict/",
        "/state_zip_to_district_lower/" + stateView.state + "/",
        "/state_zip_to_district_upper/" + stateView.state + "/"
      ];
    }
  ...
  lookUpName(name) {
    const queryRef = firestore
      ...
      .where("displayName", "==", capitalizeName(name));
    const locationData = {};
      ...
      snapshot.forEach(member => {
        ...
        if (memberData.campaigns) {
          const currentCampaign =
            memberData.campaigns[memberData.current_office_index];
        ...
        if (currentRole.level === "federal") {
          locationData.federal = {
            ...
            districts: currentRole.district
              ? [mapHelperFunctions.zeroPad(currentRole.district)]
              : []
          };
  ...
  lookUpByDistrict(district) {
    const { usState, setDistrict } = this.props;
    let locationData = {};
    ...
    if (zipCheck) {
      ...
      Promise.all(promises)
        ...
        .catch(function(error) {
          zipLookUpHandler.zipErrorResponse(
            "That zip code is not in our database, if you think this is an error please email us.",
            error
          );
        });
    } else {
  ...
  render() {
    const { dataSource, value2 } = this.state;
    const { usState } = this.props;
    const bannerSrc = usState
      ? `${location.origin}/Images/${usState}/THP_logo_inverse.png`
      : "/Images/THP_logo_inverse.png";
    return (
              ...
              >
                <div className="form-group text-center">
                  {/* <input
                    ...
                    placeholder={
                      usState
                        ? "zip, district or lawmaker"
                        : "zipcode or district"
                    }
                    onChange={this.saveZip}
                    value={this.state.query}
                  /> */}

                  <AutoComplete
                    dataSource={dataSource}
                    style={{width: "auto", display:"inline-block", verticalAlign:"middle", marginRight:"10px", marginBottom:"10px"}}
                    onSelect={this.onLawmakerNameSelect}
                    onSearch={this.onSearch}
                    placeholder="Zip, District, or Lawmaker"
                  ...
                  >
                    <Input
                      placeholder="zip district lawmaker"
                      style={{ backgroundColor: "#ff4741", color: "white", height:"46px" }}
                    />
                  </AutoComplete>

                  <input
                    type="submit"
                    className="btn btn-primary btn-lg fath-button"
                    value="Find a Town Hall"
                  />
